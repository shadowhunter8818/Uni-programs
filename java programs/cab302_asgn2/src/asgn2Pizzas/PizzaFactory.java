...
public class PizzaFactory {
	...
	public static Pizza getPizza(String pizzaCode, int quantity, LocalTime orderTime, LocalTime deliveryTime) throws PizzaException{
		...
		if (orderTime == null || deliveryTime == null){
			throw new PizzaException("Order or Delivery time is null");
		}
		if (pizzaCode == null) {
			throw new PizzaException("Pizza code cannot be null");
		} else {
			if (pizzaCode.equals("PZM")){
				P = new MargheritaPizza(quantity, orderTime, deliveryTime);
			} else {
				if (pizzaCode.equals("PZV")) {
					P = new VegetarianPizza(quantity, orderTime, deliveryTime);
					} else {
						if (pizzaCode.endsWith("PZL")) {
							P = new MeatLoversPizza(quantity, orderTime, deliveryTime);
							} else {
								throw new PizzaException("Contains a not valid pizza code");
							}
						}
				}
			}
