...
public class PizzaRestaurant {
	...
	public boolean processLog(String filename) throws CustomerException, PizzaException, LogHandlerException{
		boolean result;
		try {
			customers = LH.populateCustomerDataset(filename);
			pizzas = LH.populatePizzaDataset(filename);
			result = true;
		}
		catch (Exception e){
			result = false;
	...
	public double getTotalDeliveryDistance(){
		...
		for (int i =0 ; i < getNumCustomerOrders(); i++) {
			try{
				totDistance += getCustomerByIndex(i).getDeliveryDistance();
			}
	...
	public double getTotalProfit(){
		...
		for (int i = 0; i < getNumPizzaOrders(); i++){
			try {
				TotalProfit += getPizzaByIndex(i).getOrderProfit();
