...
void lcd_character(char c) {
	...
	for (unsigned char i = 0; i<char_width; i++) {
		for (unsigned char j = 0; j<char_height; j++) {
			set_pixel(0+i, 0+j, (pgm_read_byte(&(ASCII[c - 0x20][i])) & (1 << j)) >> j);
		}
	}

	for(unsigned int i = 0; i < LCD_BUFFER_SIZE; i++){
...
void lcd_sprite(Sprite *s) {
	...
	unsigned char dx, dy, byte_width = ceil(s->width / 8.0f);
	for (dy = 0; dy<s->height; dy++) {
		for (dx = 0; dx<s->width; dx++) {
			set_pixel(
				(unsigned char) s->x+dx,
				(unsigned char) s->y+dy,
				(s->bitmap[(int) (dy*byte_width+floor(dx/8.0f))] >> (7 - dx%8)) & 1 // ouch...
			);
		}
	}
