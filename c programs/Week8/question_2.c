...
unsigned char set_contrast(unsigned char current) {
	...
	if (current == 3) {
	lcd_write(LCD_C, 0x80 | LCD_DEFAULT_CONTRAST );
	}
	if (current == 2) {
	lcd_write(LCD_C, 0x80 | LCD_LOW_CONTRAST );
	}
	if (current == 1) {
	lcd_write(LCD_C, 0x80 | LCD_HIGH_CONTRAST );
	}
	current++;
	...
	return current; // TODO - it is not return 1...
}
...
unsigned char set_inversion(unsigned char current) {
	...
	return current; // TODO - it is not return 0....
}
