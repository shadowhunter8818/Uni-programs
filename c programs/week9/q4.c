...
int main() {
    ...
    while (1) {
        ...
        sprintf(buff, "%5.4f", get_system_time(TCNT0));
...
double get_system_time(unsigned int timer_count) {
	...
	timer_count = timer_count /10000.0 *1.28;
    return timer_count;
}
...
ISR(TIMER0_OVF_vect){
	...
	while ((PINF>>5) & 0b1) {
		btn_states[5] = BTN_STATE_DOWN;
		
	}
	if ( btn_states[5] == BTN_STATE_DOWN) {
		btn_states[5] = BTN_DPAD_UP;
		press_count++;
	}
